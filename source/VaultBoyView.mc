...
class VaultBoyView extends WatchUi.WatchFace {
    ...
    private var _frames as Array<WatchUi.BitmapResource?> = [null, null, null, null, null, null] as Array<WatchUi.BitmapResource?>;
    ...
    private const BATTERY_EVERY  = 300;
    private const BODYBATT_EVERY = 150;
    private const WEATHER_EVERY  = 300;
    ...
    function updateData() as Void {
        ...
        if (_frameCounter % BATTERY_EVERY == 0) {
            ...
            if (stats != null) {
                ...
                var sol = (stats has :solarIntensity) ? stats.solarIntensity : 0;
                if (sol != _cachedSolar) { _cachedSolar = sol; _strSolar = sol.toString(); }
            }
        }
    ...
    function drawStaticUI(dc as Graphics.Dc) as Void {
        ...
        drawVaultBar(dc, 53, 109, 102, 10, 1, 0x00FF00,  _cachedSteps.toFloat()    / 10000.0);
        dc.drawText(155, 92,  Graphics.FONT_SYSTEM_XTINY, _strSteps,    Graphics.TEXT_JUSTIFY_RIGHT);

        drawVaultBar(dc, 53, 138, 102, 10, 1, 0x00FF00,  _cachedFloors.toFloat()   / 10.0);
        dc.drawText(155, 121, Graphics.FONT_SYSTEM_XTINY, _strFloors,   Graphics.TEXT_JUSTIFY_RIGHT);

        drawVaultBar(dc, 53, 168, 102, 10, 1, 0x00FF00,  _cachedSolar.toFloat()    / 100.0);
        ...
        drawVaultBar(dc, 51, 217, 18, 12, 0, 0x00FF00,   _cachedBatt.toFloat()     / 100.0);
